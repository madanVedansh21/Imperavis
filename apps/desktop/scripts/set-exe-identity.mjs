...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'OrgHumans',
      FileDescription: 'OrgHumans',
      CompanyName: 'OrgHumans',
      LegalCopyright: 'Copyright (c) 2026 OrgHumans'
    }
  })

  console.log('[set-exe-identity] done — OrgHumans icon + identity stamped')
}
